...
app.post("/api/auth/login", (req, res) => {
  ...
  if (
    (username === "hospital1" && password === "password123") ||
    ...
    const user = {
      ...
      department:
        username === "hospital1" ? "Emergency Medicine" : "Ambulance Service",
...
io.on("connection", (socket) => {
  ...
  socket.on("transmit_vitals", (vitalsData) => {
    ...
    if (
      lastTransmission &&
      lastTransmission.patientId === vitalsData.patientId &&
      ...
      socket.emit("vitals_received", {
        status: "warning",
        message:
          "Duplicate transmission skipped - please wait 1 second between transmissions",
        timestamp: new Date().toISOString(),
    ...
    console.log(
      "📨 Sent vitals to hospital room - Transmission ID:",
      transmissionId
    );
    ...
    const interval = setInterval(() => {
      ...
      const simulatedVitals = {
        patientId:
          ...
          `PAT_SIM_${Math.random().toString(36).substr(2, 6)}`,
      ...
      const transmissionId = `${
        simulatedVitals.patientId
      }_${Date.now()}_${simulationCount}`;
      ...
      io.to("hospital_room").emit("vitals_update", classifiedVitals);
      console.log(
        `🔄 Sent simulated vitals #${simulationCount} to hospital - ID:`,
        transmissionId
      );
  ...
  socket.on("emergency_alert", (alertData) => {
    ...
    console.log(
      "📢 Emergency alert sent to hospital - ID:",
      alertWithTimestamp.alertId
    );
  ...
  socket.on("disconnect", (reason) => {
    ...
    connectedClients.hospital = connectedClients.hospital.filter(
      (id) => id !== socket.id
    );
    connectedClients.ambulance = connectedClients.ambulance.filter(
      (id) => id !== socket.id
    );
...
function classifyEmergencyLevel(vitals) {
  ...
  const systolic =
    vitals.bloodPressure?.systolic || vitals.bloodPressureSystolic;
  const diastolic =
    vitals.bloodPressure?.diastolic || vitals.bloodPressureDiastolic;
  ...
  else if (vitals.respiratoryRate < 12 || vitals.respiratoryRate > 24)
    score += 1;
