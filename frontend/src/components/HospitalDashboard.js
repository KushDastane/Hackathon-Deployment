...
const HospitalDashboard = () => {
      ...
      socketService.onVitalsUpdate((vitalsData) => {
        ...
        setPatients((prev) => {
          ...
          const existingIndex = prev.findIndex(
            (p) => p.patientId === vitalsData.patientId
          );
        ...
        setEmergencyAlerts((prev) => {
          const isDuplicate = prev.some(
            (alert) =>
              ...
              (alert.patientId === alertData.patientId &&
                alert.timestamp === alertData.timestamp)
          );
  ...
  const playAlertSound = () => {
    try {
      const audio = new Audio(
        "https://assets.mixkit.co/active_storage/sfx/250/250-preview.mp3"
      );
      audio.volume = 0.3;
  ...
  const dismissAlert = (index) => {
    ...
    if (dismissedAlert) {
      const updatedHistory = alertHistory.map((alert) =>
        alert.patientId === dismissedAlert.patientId &&
        alert.status === "active"
          ? {
              ...alert,
              status: "dismissed",
  ...
  const getVitalStatus = (patient) => {
    if (patient.heartRate < 50 || patient.heartRate > 140 || patient.spo2 < 90)
      return "critical";
    if (patient.heartRate < 60 || patient.heartRate > 120 || patient.spo2 < 95)
      return "moderate";
    return "stable";
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-teal-100 p-6">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-green-100 text-xl">
              Welcome, Dr. {user?.name} | {user?.department}
            </p>
          ...
          <div
            className={`px-5 py-3 rounded-full text-sm font-semibold shadow-md ${
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {emergencyAlerts.map((alert, index) => (
              <div
                key={alert.alertId || `alert_${index}`}
                className="bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-2xl p-5 shadow-lg animate-pulse"
              >
                <div className="flex justify-between items-start">
                  <div>
          ...
          {patients.length === 0 ? (
            <div className="bg-white rounded-2xl border-2 border-dashed border-gray-300 p-16 text-center shadow-sm">
              ...
              <p className="text-gray-500 mb-6 text-lg">
                Waiting for ambulance transmissions...
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {patients.map((patient) => {
                const emergencyLevel =
                  patient.emergencyLevel || getVitalStatus(patient);
                return (
                  <div
                    key={patient.patientId}
                    className={`bg-white rounded-2xl border-2 p-5 shadow-lg transition-all hover:shadow-xl ${getEmergencyColor(
                      emergencyLevel
                    )}`}
                  >
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">
                          ...
                          <span className="font-semibold">
                            {patient.ambulanceId || "Unknown"}
                          </span>
                        </p>
                        {patient.paramedicName && (
                          <p className="text-sm text-gray-500">
                            Paramedic: {patient.paramedicName}
                          </p>
                        )}
                        <p className="text-xs text-blue-500 mt-2">
                          Updates: {patient.updateCount || 1} | ID:{" "}
                          {patient.patientId}
                        </p>
                      </div>
                      <span
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-5">
                      {[
                        ...
                        <div
                          key={label}
                          className={`border-2 rounded-xl p-3 text-center bg-white ${color}`}
                        >
        ...
        <div>
          ...
          {alertHistory.length === 0 ? (
            <div className="bg-white rounded-2xl border-2 border-dashed border-gray-300 p-16 text-center shadow-sm">
              ...
              <p className="text-gray-500 mb-6 text-lg">
                Emergency alerts and critical vitals will appear here
              </p>
            </div>
          ) : (
      ...
      <div className="bg-white rounded-2xl border p-6 shadow-md mt-6">
        ...
        <div className="mt-4 p-4 bg-gray-50 rounded-xl text-xs text-gray-600">
          <p>
            <strong>Debug Info:</strong> Socket connected:{" "}
            {isConnected ? "Yes" : "No"} | Patients in state: {patients.length}
          </p>
          <p>
            Backend: https://ambulancemanagement-u67j.onrender.com | Last vitals
            received: {lastUpdate || "Never"}
          </p>
          <p>
            Duplicate protection: Active (processed{" "}
            {processedPatientIds.current.size} transmissions)
          </p>
        </div>
      </div>
